...
const HomeView: FC<HomeViewProps> = ({
  ...
  benefits,
  tooltipPosition,
  handleOnMouseEnterEmployeeCard,
  handleOnMouseLeaveEmployeeCard,
}) => (
  <Fragment>
    {/* Hero */}
    <main className="page-container">
        ...
        <div className="flex flex-col">
          <div className="flex flex-col gap-4 lg:gap-6">
            <div className="animate__animated animate__fadeIn duration-500" style={{ animationDelay: '1.1s' }}>
              <Chatbox title="풀타임, 파트타임" type="White" />
            </div>
            <div className="flex flex-col gap-1 lg:gap-3">
              ...
              <p className="lg:text-xl font-semibold animate__animated animate__fadeInUp duration-500">
                            ...
                            <div className="flex flex-col items-center gap-4">
                              <div className="card-profile">
                                <div className="card-profile-img">
                                  <Image src={employee.profileImage} alt="Profile Image" />
                                </div>
                                <div className="card-profile-country">
                                  <Image src={employee.countryFlag} alt="Flag Image" className="rounded-sm" />
                                </div>
                              </div>
                              <div className="card-profile-info">
                                ...
                                <p className="text-sm text-blue-500 font-medium">
                                  {employee.position}{' '}
                                  <span className="font-semibold">· {employee.lengthOfExperience}y+</span>
                                </p>
                              </div>
                            </div>
                            <div className="mt-5 card-profile-skills">
                              {employee.skills.map((skill) => (
                                <BadgeOutline key={skill.id} title={skill.name} type="Secondary" />
                              ))}
                            </div>
                          </Fragment>
                        )}
          ...
          <p className="text-sm text-[#FBFF23] underline mt-4 lg:text-white lg:text-lg font-semibold">
            개발자가 필요하신가요?
          </p>
          {achievements && (
                        ...
                        <Fragment>
                          <div className="flex flex-col items-center gap-4">
                            <div className="card-profile">
                              ...
                              <div className="card-profile-country">
                                <Image src={employee.countryFlag} alt="Country Image" className="rounded-sm" />
                              </div>
                            </div>
                            <div className="card-profile-info">
                              <p className="text-gray-950 font-semibold">{employee.name}</p>
      ...
      <div className="hidden lg:block lg:mt-6">
        <Swiper slidesPerView={6} spaceBetween={16} modules={[]}>
          {fields &&
            fields.map((field) => {
              const IconComponent = iconsMap[field.icon];
              return (
                <SwiperSlide
                  className="animate__animated animate__fadeIn duration-500"
                  style={{ opacity: 1 }}
                  key={field.id}>
                  <BadgeIcon icon={IconComponent} title={field.title} />
                </SwiperSlide>
              );
            })}
        </Swiper>
